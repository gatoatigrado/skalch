...
class BipartiteClassifier2 extends AngelicSketch {
  val tests = Array(())
...
//            val outputVal : Boolean = false
      ...
      def classify(root: Vertex): Boolean = {
        ...
        val marking: Vertex => Int = getFunction()
//        val marking: Vertex => Int = getFunctionAngelic()
        for (e <- traversal) {
        ...
        skdprint("Folding")
        val init: Boolean = !!()
        val fold: (Boolean, Edge) => Boolean = getFoldFunction()
...
object BipartiteRecognizer2 {
  def main(args: Array[String]) = {
    for (arg <- args)
      Console.println(arg)
    //val cmdopts = new cli.CliParser(args)
    skalch.AngelicSketchSynthesize(() =>
      new BipartiteClassifier2())
  }
}
