...
/* Try to find the least number of iterations needed so that everyone
 * meets everyone else. Has some predicates to ensure some increase
 * in meetings during each iteration.
 */
class MatchingFunctionSketch extends AngelicSketch {
    val tests = Array(())
    ...
    def main() {
        ...
        for (i <- 0 until iterations) {
            ...
            synthAssert(numMatched > oldNumMatched + n || numMatched == n * n);
