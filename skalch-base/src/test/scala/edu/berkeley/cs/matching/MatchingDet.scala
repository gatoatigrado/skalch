...
class MatchingDetSketch extends AngelicSketch {
    val tests = Array(())
...
//            val t : Int = !!(groups);
            ...
            for (k <- 0 until groups) {
                for (l <- 0 until groupSize) {
                    val groupId : Int = (k*s + l*x + i*y) % groups;
//                    val groupId : Int = (k + l) % groups;
                    roundGroups(groupId)((k + z) % groups) = lastRoundGroups(k)(l);
