...
// n version, updates par to any node, Emina's example (6 nodes, 12 edges), more limited angels, unscalable

class Mst11Sketch extends AngelicSketch {
  val tests = Array(())
    ...
    class MST[T](graph : Graph[T]) {
      def getMST() : Graph[T] = {
        ...
        for (i <- 0 until n) {
          ...
          for (unnode <- unnodes) {
            val nodes = graph.nodes.filter(node => mst.containsElem(node.elem))
