...
class Scan6Sketch() extends AngelicSketch {
    val tests = Array( () )
    ...
    def main() : Unit = {
        ...
        def scan(x : Array[BitVector], spec : Array[BitVector], stages : Int,
                maxAdditions : Int, fanout : Int, maxRowAdds : Int) : Array[BitVector] = { 
            ...
            for (val r : Int <- 0 until nodes(0).length) {
                for (val s : Int <- 1 until nodes.length) {
                    ...
                    if (k == r) {
                        ...
                        var sum : Int = 0
                        for (val i : Int <- 0 to r) {
                            if (nodes(s)(i)._1 != -1) {
                                sum += 1
                            }
                        }
                        synthAssert(sum <= maxRowAdds)
                    }
        ...
        val mid : Array[Int] = Array(1,3,4,15,16,48,64,255)
        //        ,256,768,1024,3840,4096,12288,16384,65535)
        ...
        val stages : Int = 7
        val maxAdditions : Int = 8*2
        val fanout : Int = 2
        val maxRowAdds : Int = 4
        skdprint("in main")
        ...
        var r2 : Array[BitVector] = scan(vectorInput, spec, stages, maxAdditions, fanout, maxRowAdds)
        print1DArray(r2)
    }
}
