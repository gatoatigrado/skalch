...
/*
 * Angelic traversal through graph. And it angelically merges the 
 * connected components of two nodes (different from Scc2)
 */
class SccSketch3() extends AngelicSketch {
    val tests = Array( () )
    ...
    def main() : Unit = {
        ...
        class EdgeTraversal(forward : Boolean, vertex : Vertex) {
            ...
            private def next() : Edge = {
                ...
                val nextEdgeIndex : Int = !!(availableEdges.size)
        ...
        class Graph {
            ...
            def merge(v1 : Vertex, v2 : Vertex) {
                ...
                val useV1Representative : Boolean = !!()
                skput_check(useV1Representative)
                if (useV1Representative) {
                    changedVertex.setRepresentative(v1.getRepresentative())
                } else {
                    changedVertex.setRepresentative(v2.getRepresentative())
                }
            }
