 ...
 * Added DFS numbering and tried to figure out what predicates are required to merge.
 */
class SccSketch6() extends AngelicSketch {
    val tests = Array( () )
...
//            private var representative : Vertex = this
            ...
            def setDfsNumber(num : Int) {
                if (dfsNumberIn == -1) {
                    dfsNumberIn = num
                } else if (dfsNumberOut == -1) {
                    dfsNumberOut = num
                }
            }
...
//            }
        ...
        class Graph {
            ...
            def mark() {
                ...
                while (edge != null) {
                    setDfsNumber(edge.src)
                    skdprint("Visiting edge: " + edge)
                    
                    val destDfsNum : Int = edge.dest.getDfsNumberIn
                    val destHasDfsNum : Boolean = destDfsNum != -1
                    val isInSameClass : Boolean = isSameClass(edge.src, edge.dest)
                    
                    var shouldMerge : Boolean = false
                    val choice : Int = !!(List(0,1,2,3,4))
                    skdprint("Same class: " + isInSameClass + ", DfsNum: " + destDfsNum);
                    choice match {
                        case 0 => shouldMerge = destHasDfsNum
                        case 1 => shouldMerge = destHasDfsNum && destDfsNum < curDfsNum
                        case 2 => shouldMerge = !destHasDfsNum
            ...
            def setDfsNumber(vertex : Vertex) {
                vertex.setDfsNumber(curDfsNum)
                curDfsNum += 1
            }
