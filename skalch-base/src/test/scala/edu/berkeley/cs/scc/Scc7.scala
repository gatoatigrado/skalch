 ...
 */
class SccSketch7() extends AngelicSketch {
    val tests = Array( () )
...
//            }
        ...
        class Graph {
            ...
            def mark() {
                ...
                var prevVertices : List[Vertex] = List(null);
                
                visitVertex(prevVertices.head, root)
                setDfsNumberIn(root)
                prevVertices ::= root;
                ...
                while (edge != null) {
                    visitVertex(edge.src, edge.dest)
                    setDfsNumberIn(edge.dest)
                    prevVertices ::= edge.dest
                    
                    skdprint("Visiting edge: " + edge)
                    
                    edge = traversal.getEdge()
                    while (edge != null && prevVertices.head != edge.src) {
                        //visitVertex(prevVertices.tail.head, prevVertices.head)
                        skdprint("Backtracking: " + prevVertices.head)
                        setDfsNumberOut(prevVertices.head)
                        prevVertices = prevVertices.tail
                    }
                }
                
                while (prevVertices.tail != Nil) {
                    skdprint("Backtracking: " + prevVertices.head)
                    setDfsNumberOut(prevVertices.head)
                    prevVertices = prevVertices.tail
                }
            ...
            def visitVertex(prevVertex : Vertex, curVertex : Vertex) {
                ...
                val choice : Int = !!(List(0,1,2,3,4))
                skdprint("Same class: " + isInSameClass + ", DfsNum: " + dfsNum);
                choice match {
                    case 0 => shouldMerge = hasDfsNum
                    case 1 => shouldMerge = hasDfsNum && dfsNum < curDfsNum
                    case 2 => shouldMerge = !hasDfsNum
                    case 3 => shouldMerge = isInSameClass
                    case 4 => shouldMerge = !isInSameClass
                }
