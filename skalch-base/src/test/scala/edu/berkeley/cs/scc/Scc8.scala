...
/*
 * Does DFS traversal of graph. Rewrote traveresal.
 */
class SccSketch8() extends AngelicSketch {
    val tests = Array( () )
            ...
            def dfs(prevVertex : Vertex, vertex : Vertex, up : Boolean) {
                ...
                for (nextVertex <-vertex.getConnectedVertices) {
                    if (nextVertex.getDfsNumberIn == -1) {
                        ...
                        skdprint("Already visited: " + nextVertex)
                        visitVertex(prevVertex, nextVertex)
                    }
                }
