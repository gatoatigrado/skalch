...
class TopoSortSketch extends AngelicSketch {
    val tests = Array( () )
    ...
    def main() {
        ...
        class Graph {
            ...
            def markNodes() {
                    dfs(root, new ListBuffer[Vertex])
            }
            ...
            def dfs(v : Vertex, visited : ListBuffer[Vertex]) {
                skdprint("Vertex: " + v + "; Visited: " + visited)
                
                if (visited.contains(v)) {
                    val markNode : Boolean = !!()
                    if (markNode) {
                        setNode(v, visited)
                        skdprint("Marked node: " + v.id + " = " + v.sort )
                    }
                    skdprint("Pop")
                    return
                } 
